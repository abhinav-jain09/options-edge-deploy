# Requirement Intake Portal — dedicated Bugzilla at req.bleadingoptions.com with Keycloak SSO (realm-isolated)
...
1. log in at **https://req.bleadingoptions.com** through **Keycloak** (`auth.bleadingoptions.com`),
2. belong to a **separate Keycloak realm** (`req`) whose users **cannot** authenticate to the
   OptionsEdge trading UI/API (`bleadingoptions.com`, realm `optionsedge`), and
...
| Cloudflare tunnel `options-edge-option-chain` (`/etc/cloudflared/options-edge-stable.yml`): `bleadingoptions.com`→`.252:8094` (+`/ws/events`→`:30097`), `auth.bleadingoptions.com`→`10.43.127.26:8080` (`/admin`,`/realms/master`→404), `es.bleadingoptions.com`→`.4:30080`, catch-all 404 | config read |
...
["https://req.bleadingoptions.com/oidc-callback"]` — exact URI, no wildcard**, `webOrigins:
["https://req.bleadingoptions.com"]`, no post-logout redirect URIs. **Claim issuance is part of this
...
- Ingress rule (before catch-all): `hostname: req.bleadingoptions.com` → `service: http://127.0.0.1:8093`
  (loopback; cloudflared runs on `.252`). Config backed up (`.bak-<ts>`); restart in REQ-10 window.
- **Pre-check:** before enabling the ingress rule, verify NO `req.bleadingoptions.com` DNS record
...
- **Rollback — two branches:**
  ...
  - *Public fail-closed invariant (identical wording governs §6):* (1) point the
    `req.bleadingoptions.com` ingress rule at `http_status:404` (or remove it); (2) restart cloudflared;
...
- **Two separate listeners (structural, distinct container ports — not ServerName-based):**
  ...
  port 8094 must NOT be used — the existing tunnel routes `bleadingoptions.com` → `.252:8094`** (§3);
...
- **Public vhost OIDC directives:** `OIDCProviderMetadataURL
  https://auth.bleadingoptions.com/realms/req/.well-known/openid-configuration`, `OIDCClientID
  bugzilla-web`, `OIDCRedirectURI https://req.bleadingoptions.com/oidc-callback`, **`OIDCScope "openid
...
- Params: `urlbase=https://req.bleadingoptions.com/`, `user_info_class=Env,CGI`, `auth_env_*` per 5c,
...
- **V6t:** disposable test user + TEMPORARY kcadm test client (`isolation-test`, direct-grant,
  ...
  `bleadingoptions.com` ⇒ no session; symmetric negative for `optionsedge` users at the portal. Client
...
- **Layer 3 (edge, best-effort within plan):** at implementation, RECORD the current Cloudflare
  ...
  `auth.bleadingoptions.com/realms/req/*` login POSTs — path-scoped so realm `optionsedge` is untouched
  (V11b runs IMMEDIATELY when this rule lands, §6 step 6); (b) `req.bleadingoptions.com` handshake
...
| V6t | disposable `req` token → trading API; `req` session → `bleadingoptions.com`; cleanup verified | 401/403; no session; test client deleted | no |
...
| V8 | all hostnames after each cloudflared restart, explicitly incl. `bleadingoptions.com` → `.252:8094` (the route adjacent to the portal's port choice) and `bleadingoptions.com/ws/events` WebSocket | normal | no |
...
26 G memory available, host ports 8093 and 8095 both free, and NO `req.bleadingoptions.com` DNS record
...
admin `:8089` → 302 (V9 complete); `https://bleadingoptions.com/` returned 200 and the `optionsedge`
