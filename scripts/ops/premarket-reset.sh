...
PRESERVE_TOPICS_REGEX='^(es\.)?options\.spx\.gamma-ladder-path\.(state|events)$|^es\.reversal\.(final-summary|outcome)$|^es\.reversal\.hunt\.(state|alerts)$|^es\.reversal\.hunt\.swing\.candidates$|^options\.spx\.wall-break-rates\.dataset$|^(es|spx)\.drop\.(final-summary|outcome)$|^spx\.basis\.state$|^underlying\.vix\.price$|^options\.spx\.gamma-migration\.scoring|^(es\.)?options\.spx\.gamma-migration\.(crossings|wall-touch|fragility)$|corridor-gauge-event-log$|^options\.databento\.oi\.anchor-manifest$|^es\.tape-zones\.board$|gamma-migration-scorer-changelog$|^context-tape\.direction\.ledger$|^es\.futures\.footprint\.strike$|^options\.spx\.vol-premium\.(ivrv|events|warnings|baseline)$'
