#!/usr/bin/env bash
# The es-auction-mirror job's SOURCE/TARGET shape assertions, tested against mocked kafka CLIs.
#
...
JOB="Jenkinsfile.es-auction-mirror"
WORK="$(mktemp -d)"
...
good() {
  MOCK_TOPIC=es.futures.auction PARTS=1 POLICY=delete RET=604800000
...
check "matching source+target on the DEV target passes" 0 "target es.futures.auction verified: 1 partition(s), delete, retention.ms=604800000"
good; TGT_BROKER=192.168.100.252:9092
check "matching source+target on the PROD target passes" 0 "target es.futures.auction verified"
TGT_BROKER=127.0.0.1:19092

# -- SOURCE drift fails BEFORE the target is touched ---------------------------------------------
good; SRC_POLICY=compact,delete
check "SOURCE at compact,delete FAILS" 1 "source es.futures.auction cleanup.policy=compact,delete, frozen contract says delete"
...
check "SOURCE at pure compact FAILS" 1 "source es.futures.auction cleanup.policy=compact, frozen contract says delete"
good; SRC_RET=-1
check "SOURCE retention drift (unlimited) FAILS" 1 "source es.futures.auction retention.ms=-1, frozen contract says 604800000"
good; SRC_RET=43200000
check "SOURCE retention drift (12h es4 default) FAILS" 1 "source es.futures.auction retention.ms=43200000"
good; SRC_PARTS=4
check "SOURCE partition drift FAILS" 1 "source es.futures.auction partition count does not match frozen contract PARTS=1"
...
good; TGT_PARTS=32
check "PRE-EXISTING target at the dev 32-partition default FAILS" 1 "target es.futures.auction exists with wrong partition count (want 1)"
...
check "target still compact,delete AFTER reconcile FAILS" 1 "target es.futures.auction cleanup.policy=compact,delete, frozen contract says delete"
touched_target || { echo "  FAIL a policy-drifted target was never reconciled (no --alter)"; fail=1; }
good; TGT_POLICY=compact
check "target at pure compact FAILS" 1 "target es.futures.auction cleanup.policy=compact, frozen contract says delete"
good; TGT_RET=-1
check "target retention drift (unlimited) FAILS" 1 "target es.futures.auction retention.ms=-1, frozen contract says 604800000"
good; TGT_RET=86400000
check "target retention drift (24h env default) FAILS" 1 "target es.futures.auction retention.ms=86400000"
good; ALTER_RC=1
check "reconcile itself failing FAILS the install" 1 "cannot reconcile es.futures.auction config"
...
[ $fail -eq 0 ] && echo "=== es-auction-mirror-shape-test: OK ===" || { echo "=== es-auction-mirror-shape-test: FAILED ==="; exit 1; }
