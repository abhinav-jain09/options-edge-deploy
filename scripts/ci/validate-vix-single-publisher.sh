...
# JSON, or an unreadable replicas field FAIL CLOSED.
#
...
spx_owner = False
spx_detail = f"{SPX_DEPLOYMENT}: Deployment absent from the production render"
if spx is not None:
    enabled_present, enabled_value = env_value(spx, SPX_DEPLOYMENT, "DATABENTO_VIX_PRICE_ENABLED")
    enabled = enabled_present and enabled_value.strip().lower() in {"1", "true", "yes", "on"}
    ...
    if extra_present and extra_value.strip():
        ...
        for instrument in instruments:
            ...
            if instrument.get("expiry_mode") == "AUTO_VIX_MONTHLY":
...
standalone_detail = f"{STANDALONE_DEPLOYMENT}: Deployment absent from the production render"
