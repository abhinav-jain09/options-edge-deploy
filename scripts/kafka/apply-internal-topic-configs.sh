...
with_timestamp_after_max() {
  local config="$1"
  if [[ -n "$MESSAGE_TIMESTAMP_AFTER_MAX_MS" ]]; then
    config="$config,message.timestamp.after.max.ms=$MESSAGE_TIMESTAMP_AFTER_MAX_MS"
  fi
  echo "$config"
}
...
apply_changelog_config() {
  local topic="$1"
  local config
  config="$(with_timestamp_after_max "cleanup.policy=[compact,delete],retention.ms=$RETENTION_MS,segment.ms=$SEGMENT_MS,delete.retention.ms=$DELETE_RETENTION_MS,min.cleanable.dirty.ratio=$MIN_CLEANABLE_DIRTY_RATIO,min.insync.replicas=$MIN_ISR")"
...
apply_repartition_config() {
  local topic="$1"
  local config
  config="$(with_timestamp_after_max "cleanup.policy=delete,retention.ms=$RETENTION_MS,segment.ms=$SEGMENT_MS,min.insync.replicas=$MIN_ISR")"
