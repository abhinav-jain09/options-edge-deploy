...
class Es4TopicPartitionContractTest(unittest.TestCase):
    def test_incident_topics_are_declared_at_their_real_shape(self):
        declared = _declared()
    ...
    def test_source_topic_of_the_wedged_classifier_is_not_under_declared(self):
    ...
    def test_every_declaration_is_well_formed_and_unique(self):
        seen = {}
        for m in re.finditer(r'^OPTIONS_EDGE_ES4_TOPICS="([^"]*)"', ENV, re.M):
            for entry in m.group(1).split():
                if entry.startswith("$"):
    ...
    def test_verify_passes_when_live_matches_the_contract(self):
    ...
    def test_steady_mode_fails_on_a_wider_than_declared_topic(self):
    ...
    def test_steady_failure_names_both_remedies_rather_than_guessing(self):
    ...
    def test_created_mode_does_not_adjudicate_width(self):
    ...
    def test_missing_topic_is_fatal_in_both_modes(self):
        for mode in ("created", "steady"):
    ...
    def test_narrower_topic_is_fatal_in_both_modes(self):
        for mode in ("created", "steady"):
    ...
    def test_describe_failure_is_fatal_even_with_partial_output(self):
    ...
    def test_empty_describe_is_fatal(self):
        r = _run_verify("steady", "")
        assert r.returncode != 0
        assert "UNREACHABLE" in r.stderr

    def test_unparseable_describe_line_is_fatal(self):
    ...
    def test_escape_hatch_covers_drift_only(self):
    ...
    def test_topic_name_matching_is_anchored(self):
    ...
    def test_verify_script_syntax_and_permissions(self):
    ...
    def test_clean_reset_runs_created_before_restore_and_steady_after(self):
    ...
    def test_post_restore_findings_are_fatal_not_advisory(self):
    ...
    def test_wedge_scan_cannot_read_an_error_as_a_pass(self):
    ...
    def test_create_topics_job_runs_the_steady_audit(self):
    ...
    def test_settle_seconds_is_validated_not_passed_straight_to_sleep(self):
    ...
    def test_pod_discovery_failure_is_fatal(self):
    ...
    def test_wedge_scan_is_time_bounded_not_line_bounded(self):
    ...
    def test_wedges_and_unreadable_pods_are_reported_independently(self):
    ...
    def test_audits_wait_on_the_named_rollouts_and_die_on_timeout(self):
    ...
    def test_scan_window_is_recomputed_per_pod(self):
    ...
    def test_jenkins_uses_the_same_neutral_semantics_as_the_verifier(self):
    ...
    def test_identical_duplicate_declaration_is_rejected_when_executed(self):
    ...
    def test_conflicting_duplicate_declaration_is_rejected_when_executed(self):
        r = _run_verify_with_declaration("es.a:4 es.a:32")
        assert r.returncode != 0
        assert "DUPLICATE declaration for es.a" in r.stderr

    def test_malformed_declaration_is_rejected_before_any_broker_call(self):
    ...
    def test_a_single_well_formed_declaration_still_passes(self):
        """Guards the three tests above against passing for the wrong reason."""
        r = _run_verify_with_declaration("es.a:4")
        assert r.returncode == 0, r.stdout + r.stderr

    def test_every_scan_kubectl_call_is_request_bounded(self):
    ...
    def test_rollout_timeout_is_validated(self):
