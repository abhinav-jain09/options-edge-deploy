...
class HpsfReplayReportGateTest(unittest.TestCase):
    ...
    def test_JenkinsReplayScriptArchivesReportTest(self) -> None:
        ...
        self.assertIn('Replay image manifest missing expected platform ${HPSF_REPLAY_BUILD_PLATFORM}', pipeline)
        ...
        self.assertIn("string(name: 'REPLAY_TOPIC_PARTITIONS', defaultValue: '12'", pipeline)
        self.assertIn("string(name: 'HPSF_STAGE_A_REPLAY_STREAM_THREADS', defaultValue: '6'", pipeline)
        self.assertIn("Keep at 12 for the Stage A replay tuning ladder", pipeline)
        self.assertIn("With 12 partitions use 6 as the starting point", pipeline)
        self.assertIn("try 8 only if lag remains high and CPU is not saturated", pipeline)
        self.assertIn("use 12 only if CPU/GC remain healthy", pipeline)
        self.assertIn("string(name: 'HPSF_STAGE_A_REPLAY_CACHE_MAX_BYTES_BUFFERING', defaultValue: '134217728'", pipeline)
        self.assertIn("string(name: 'HPSF_STAGE_A_REPLAY_COMMIT_INTERVAL_MS', defaultValue: '5000'", pipeline)
        self.assertIn("string(name: 'HPSF_STAGE_A_REPLAY_PROCESSING_GUARANTEE', defaultValue: 'at_least_once'", pipeline)
        self.assertIn('export REPLAY_TOPIC_PARTITIONS="${REPLAY_TOPIC_PARTITIONS:-12}"', pipeline)
        self.assertEqual(2, pipeline.count("--env=HPSF_STREAMS_PROCESSING_GUARANTEE=at_least_once"))
        self.assertEqual(1, pipeline.count("--env=HPSF_STREAMS_NUM_STREAM_THREADS=1"))
        self.assertEqual(2, pipeline.count("--env=HPSF_STREAMS_CACHE_MAX_BYTES_BUFFERING=0"))
        self.assertIn('--env=HPSF_STREAMS_PROCESSING_GUARANTEE="${HPSF_STAGE_A_REPLAY_PROCESSING_GUARANTEE:-at_least_once}"', pipeline)
        self.assertIn('--env=HPSF_STREAMS_NUM_STREAM_THREADS="${HPSF_STAGE_A_REPLAY_STREAM_THREADS:-6}"', pipeline)
        self.assertIn("string(name: 'HPSF_STAGE_B_REPLAY_STREAM_THREADS', defaultValue: '6'", pipeline)
        self.assertIn('--env=HPSF_STREAMS_NUM_STREAM_THREADS="${HPSF_STAGE_B_REPLAY_STREAM_THREADS:-6}"', pipeline)
        ...
        self.assertIn("string(name: 'HPSF_STAGE_A_REPLAY_CPU_REQUEST', defaultValue: '4'", pipeline)
        self.assertIn("string(name: 'HPSF_STAGE_A_REPLAY_CPU_LIMIT', defaultValue: '8'", pipeline)
        self.assertIn("string(name: 'HPSF_STAGE_A_REPLAY_MEMORY_REQUEST', defaultValue: '8Gi'", pipeline)
        self.assertIn("string(name: 'HPSF_STAGE_A_REPLAY_MEMORY_LIMIT', defaultValue: '12Gi'", pipeline)
        self.assertIn("string(name: 'HPSF_STAGE_A_REPLAY_EPHEMERAL_STORAGE_REQUEST', defaultValue: '10Gi'", pipeline)
        self.assertIn("string(name: 'HPSF_STAGE_A_REPLAY_EPHEMERAL_STORAGE_LIMIT', defaultValue: '20Gi'", pipeline)
        self.assertIn("string(name: 'HPSF_STAGE_A_REPLAY_JAVA_TOOL_OPTIONS', defaultValue: '-Xms3g -Xmx8g -XX:+UseG1GC -XX:MaxGCPauseMillis=200 -XX:+ExitOnOutOfMemoryError'", pipeline)
        self.assertIn('stage_a_java_tool_options="${HPSF_STAGE_A_REPLAY_JAVA_TOOL_OPTIONS:--Xms3g -Xmx8g -XX:+UseG1GC -XX:MaxGCPauseMillis=200 -XX:+ExitOnOutOfMemoryError}"', pipeline)
        self.assertIn('--env=JAVA_TOOL_OPTIONS="$stage_a_java_tool_options"', pipeline)
        self.assertIn("--dry-run=client -o json > artifacts/stage-a-pod.json", pipeline)
        self.assertIn('"cpu": os.environ.get("HPSF_STAGE_A_REPLAY_CPU_REQUEST", "4")', pipeline)
        self.assertIn('"cpu": os.environ.get("HPSF_STAGE_A_REPLAY_CPU_LIMIT", "8")', pipeline)
        self.assertIn('"memory": os.environ.get("HPSF_STAGE_A_REPLAY_MEMORY_REQUEST", "8Gi")', pipeline)
        self.assertIn('"memory": os.environ.get("HPSF_STAGE_A_REPLAY_MEMORY_LIMIT", "12Gi")', pipeline)
        self.assertIn('"ephemeral-storage": os.environ.get("HPSF_STAGE_A_REPLAY_EPHEMERAL_STORAGE_REQUEST", "10Gi")', pipeline)
        self.assertIn('"ephemeral-storage": os.environ.get("HPSF_STAGE_A_REPLAY_EPHEMERAL_STORAGE_LIMIT", "20Gi")', pipeline)
        self.assertIn('kubectl -n "$NAMESPACE" create -f artifacts/stage-a-pod.json', pipeline)
        self.assertIn("string(name: 'HPSF_STAGE_B_REPLAY_CPU_REQUEST', defaultValue: '6'", pipeline)
        self.assertIn("string(name: 'HPSF_STAGE_B_REPLAY_CPU_LIMIT', defaultValue: '10'", pipeline)
        self.assertIn("string(name: 'HPSF_STAGE_B_REPLAY_MEMORY_REQUEST', defaultValue: '12Gi'", pipeline)
        self.assertIn("string(name: 'HPSF_STAGE_B_REPLAY_MEMORY_LIMIT', defaultValue: '16Gi'", pipeline)
        self.assertIn("string(name: 'HPSF_STAGE_B_STRIKE_FLOW_CONSUMED_LOG_ENABLED', defaultValue: 'false'", pipeline)
        self.assertIn('--env=HPSF_STAGE_B_STRIKE_FLOW_CONSUMED_LOG_ENABLED="${HPSF_STAGE_B_STRIKE_FLOW_CONSUMED_LOG_ENABLED:-false}"', pipeline)
        self.assertIn("--dry-run=client -o json > artifacts/stage-b-pod.json", pipeline)
        self.assertIn('"cpu": os.environ.get("HPSF_STAGE_B_REPLAY_CPU_REQUEST", "6")', pipeline)
        self.assertIn('"cpu": os.environ.get("HPSF_STAGE_B_REPLAY_CPU_LIMIT", "10")', pipeline)
        ...
        self.assertIn("Stage B legacy startup markers not present in recent logs; RUNNING state marker is the readiness gate.", pipeline)
        ...
        self.assertIn("stage_b_full_log=\"$(mktemp)\"", pipeline)
        self.assertIn("grep 'HPSF_STAGE_B_PERFORMANCE_METRICS_JSON=' \"$stage_b_full_log\" > artifacts/logs/stage-b-metrics.log || true", pipeline)
        self.assertIn("tail -2000 \"$stage_b_full_log\" > artifacts/logs/stage-b-runtime.log || true", pipeline)
        self.assertIn("cat artifacts/logs/stage-b-startup.log artifacts/logs/stage-b-metrics.log artifacts/logs/stage-b-runtime.log > artifacts/logs/stage-b.log", pipeline)
